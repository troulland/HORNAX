...
onMounted(() => { if (store.history.length === 0) store.fetchHistory(100) })
...
const match = computed(() => store.history.find(m => m.id === Number(route.params.id)) ?? null)

const rd = computed<RiotData | null>(() => {
  const raw = match.value?.riot_data
  if (!raw) return null
...
<template>
  <div class="gv">
    <!-- Nav -->
    <div class="gv__nav">
      <button class="gv__back" @click="router.push({ name: 'analytics' })">
        <ArrowLeft :size="15" />
        <span>Analytics</span>
      </button>
        ...
        <div class="gv__radar-card">
          ...
          <svg :viewBox="`0 0 240 240`" class="gv__radar-svg">
            ...
            <polygon v-for="l in [0.25,0.5,0.75,1]" :key="l" :points="ring(l)"
              fill="none" stroke="#1A1F2E" stroke-width="1" />
            <!-- Axes -->
            <line v-for="i in 6" :key="i"
              :x1="CX" :y1="CY" :x2="axisEnd(i-1).split(',')[0]" :y2="axisEnd(i-1).split(',')[1]"
              stroke="#1A1F2E" stroke-width="1" />
            <!-- Team avg -->
            <polygon :points="avgPolygon" fill="rgba(100,116,139,0.15)" stroke="#3D4460" stroke-width="1.5" stroke-dasharray="3,2" />
            <!-- User -->
...
.gv__back {
  ...
  background: none; border: 1px solid #1A1F2E; border-radius: 6px;
  color: #8892B0; font-family: 'Rajdhani', sans-serif; font-size: 12px;
...
.gv__nav-queue { font-family: 'Rajdhani', sans-serif; font-size: 11px; font-weight: 700; letter-spacing: 1.5px; color: #8892B0; background: #1A1F2E; border-radius: 4px; padding: 3px 8px; }
.gv__nav-dur, .gv__nav-date { font-family: 'Inter', sans-serif; font-size: 11px; color: #3D4460; }

/* Empty */
.gv__empty { display: flex; flex-direction: column; align-items: center; gap: 12px; padding: 60px; background: #111520; border: 1px solid #1A1F2E; border-radius: 10px; font-family: 'Rajdhani', sans-serif; font-size: 13px; font-weight: 600; letter-spacing: 2px; color: #3D4460; text-align: center; }
...
/* Radar */
.gv__radar-card { background: #111520; border: 1px solid #1A1F2E; border-radius: 10px; padding: 16px; display: flex; flex-direction: column; align-items: center; gap: 10px; }
.gv__card-title { font-family: 'Rajdhani', sans-serif; font-size: 10px; font-weight: 700; letter-spacing: 2px; color: #3D4460; align-self: flex-start; }
...
.gv__legend-item--avg { color: #3D4460; }
.gv__legend-item--avg::before { background: #3D4460; border-style: dashed; }
...
.gv__highlight-name { font-family: 'Rajdhani', sans-serif; font-size: 14px; font-weight: 700; color: #EEF2FF; letter-spacing: 1px; }
.gv__highlight-name--user { color: var(--accent); }
.gv__highlight-champ-name { font-family: 'Inter', sans-serif; font-size: 10px; color: #3D4460; }
...
.gv__highlight-stats { display: flex; gap: 10px; font-family: 'Rajdhani', sans-serif; font-size: 11px; font-weight: 600; letter-spacing: .5px; color: #8892B0; }

/* Team scores */
.gv__team-scores { background: #111520; border: 1px solid #1A1F2E; border-radius: 8px; padding: 12px 14px; display: flex; flex-direction: column; gap: 8px; flex: 1; }
...
.gv__tscore-name { font-family: 'Rajdhani', sans-serif; font-size: 12px; font-weight: 700; color: #8892B0; letter-spacing: .5px; width: 80px; flex-shrink: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.gv__tscore-name--user { color: var(--accent); }
.gv__tscore-bar-wrap { flex: 1; height: 5px; background: #1A1F2E; border-radius: 3px; overflow: hidden; }
...
.gv__chart-card { background: #111520; border: 1px solid #1A1F2E; border-radius: 10px; padding: 16px; display: flex; flex-direction: column; gap: 12px; }
...
.gv__bar-name { font-family: 'Rajdhani', sans-serif; font-size: 11px; font-weight: 700; color: #8892B0; width: 76px; flex-shrink: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.gv__bar-name--user   { color: var(--accent); }
.gv__bar-name--enemy  { color: #3D4460; }
.gv__bar-track { flex: 1; height: 6px; background: #1A1F2E; border-radius: 3px; overflow: hidden; }
.gv__bar-fill { height: 100%; border-radius: 3px; transition: width .4s ease; }
.gv__bar-val { font-family: 'Rajdhani', sans-serif; font-size: 12px; font-weight: 700; color: #8892B0; width: 36px; text-align: right; flex-shrink: 0; }
.gv__bar-val--gold { color: #F59E0B; }

/* Row 3 - Scoreboard */
.gv__scoreboard { display: flex; flex-direction: column; gap: 12px; }
.gv__team-block { background: #111520; border: 1px solid #1A1F2E; border-radius: 10px; overflow: hidden; }
...
.gv__sb-header {
  ...
  padding: 6px 16px;
  font-family: 'Rajdhani', sans-serif; font-size: 9px; font-weight: 700; letter-spacing: 1.5px; color: #3D4460;
...
.sb-pname { font-family: 'Rajdhani', sans-serif; font-size: 13px; font-weight: 700; color: #EEF2FF; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.sb-pname--user { color: var(--accent); }
.sb-cname { font-family: 'Inter', sans-serif; font-size: 9px; color: #3D4460; white-space: nowrap; }
.sb-role { font-family: 'Rajdhani', sans-serif; font-size: 9px; font-weight: 700; letter-spacing: 1px; color: #8892B0; background: #1A1F2E; border-radius: 3px; padding: 1px 5px; flex-shrink: 0; }
.sb-kda { font-family: 'Rajdhani', sans-serif; font-size: 13px; font-weight: 700; color: #EEF2FF; }
.sb-ratio { font-family: 'Rajdhani', sans-serif; font-size: 12px; font-weight: 700; }
.sb-num { font-family: 'Rajdhani', sans-serif; font-size: 13px; font-weight: 600; color: #8892B0; text-align: center; }
.sb-gold { color: #F59E0B; }
.sb-col-d { display: flex; flex-direction: column; gap: 3px; padding-right: 8px; }
.sb-dmg { font-family: 'Rajdhani', sans-serif; font-size: 12px; color: #8892B0; }
.sb-dmg-bar { height: 4px; background: #1A1F2E; border-radius: 2px; overflow: hidden; }
...
.obj-sep { color: #1A1F2E; font-size: 14px; }
.obj-score--win  { color: var(--accent); }
.obj-score--loss { color: #3D4460; }
.obj-score--even { color: #8892B0; }

.obj-bar {
  width: 100%; height: 3px;
  background: #1A1F2E; border-radius: 2px; overflow: hidden; margin-top: 2px;
}
.obj-bar__my {
  height: 100%;
...
.w-name {
  ...
  color: #8892B0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
}
.w-name--user { color: var(--accent); }
.w-val { font-family: 'Rajdhani', sans-serif; font-size: 14px; font-weight: 700; color: #EEF2FF; }
