...
export function getTeams(_req: Request, res: Response): void {
  const teams = db.prepare(`
    ...
    GROUP BY t.id
  `).all() as unknown as (Team & { current_players: number })[]
...
export function getTeamRoster(req: Request, res: Response): void {
  const team = db.prepare('SELECT * FROM teams WHERE id = ?').get(req.params.id) as unknown as Team | undefined
